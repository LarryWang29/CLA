...
def Get_Error1(A):
    ...
    :return Error1: an array of errors for Householder algorithm
    """
    N = np.shape(A)[1]
    Error, Error1 = [], []
    for i in range(100):
    ...
    plt.plot(x_arr, np.log10(Error), label='$\log_{10}$(MGS Error)')
    plt.plot(x_arr, np.log10(Error1), label='$\log_{10}$(Householder Error)')
